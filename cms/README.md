...
## Файлы: публичные и приватные

`access.mjs` создаёт публичную папку (фикс. id) и **ограничивает публичное
чтение файлов только ею**. Фото каталога загружаются в эту папку (доступны
через `/assets/:id`), а материалы и вложения сделок — вне её (приватные).
Приватные файлы кабинет отдаёт через авторизованный прокси `/cabinet/file/[id]`.

## Прод: сервисный токен (а не админ)

Для серверной записи лидов/подписок на проде создайте **отдельного
пользователя** с минимальными правами (create `leads`/`subscribers`, чтение
`subscribers` для уникальности промокода) и выпишите ему статический токен →
`DIRECTUS_ADMIN_TOKEN` в `deploy/.env`. Админ-токен на проде не используйте.
